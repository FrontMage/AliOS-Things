...
{
    pi_gpio_pin_configure(0, 1<<GPIOA0_LED      , PI_GPIO_OUTPUT);
    pi_gpio_pin_configure(0, 1<<GPIOA1          , PI_GPIO_INPUT);
    pi_gpio_pin_configure(0, 1<<GPIOA2_NINA_RST , PI_GPIO_OUTPUT);
    pi_gpio_pin_configure(0, 1<<GPIOA3_CIS_EXP  , PI_GPIO_OUTPUT);
    pi_gpio_pin_configure(0, 1<<GPIOA4_1V8_EN   , PI_GPIO_OUTPUT);
    pi_gpio_pin_configure(0, 1<<GPIOA5_CIS_PWRON, PI_GPIO_OUTPUT);
    pi_gpio_pin_configure(0, 1<<GPIOA18         , PI_GPIO_INPUT);
    pi_gpio_pin_configure(0, 1<<GPIOA19         , PI_GPIO_INPUT);
    pi_gpio_pin_configure(0, 1<<GPIOA21_NINA17  , PI_GPIO_OUTPUT);
...
#ifdef PERF_COUNT
        unsigned int tm = pi_time_get_us();
#endif
        ...
        if(ClusterDetectionCall.num_reponse)
        {
            ...
            if(cascade_history_size == FACE_DETECTOR_STABILIZATION_PERIOD)
            {
                if(check_detection_stability(cascade_history, FACE_DETECTOR_STABILIZATION_PERIOD))
                {
                    ...
                    File = pi_bridge_open(string_buffer, O_RDWR | O_CREAT, S_IRWXU, NULL);
                    if (File == 0)
                    {
                        PRINTF("Failed to open file, %s\n", string_buffer);
                        pmsis_exit(-8);
                    }
                    pi_bridge_write(File,  ClusterDnnCall.output, 512*sizeof(short), NULL);
                    pi_bridge_close(File, NULL);

...
typedef struct {
	...
	AT_MemLocation_T ExecLoc;		/* Where the arg is located when used by a kernel, can be different than HomeLoc */
...
typedef struct AGraphEdgeWeb_T {
	...
	Kernel_Arg_T *KerArg;
	GraphNodeList_T *From;
	GraphNodeList_T *To;
	GraphEdgeWebList_T *Equiv;
	GraphEdgeWeb_T *Next;
	SymbolAlloc_T *Alloc;
} GraphEdgeWeb_T;
...
typedef struct AGraphNode_T {
	...
	union {
		Kernel_T *UK;		/* User kernel */
		KernelGroup_T *UK_Group;/* User kernel group */
	} Obj;
	...
	MemHierarchy_T Memory[MEM_LAST];
	MemHierarchy_T OwnMemory[MEM_LAST];
